...
RSpec.describe SessionsController, type: :controller do
	...
	describe "POST /sessions" do
		it "assigns a new user object" do
			post :create, params:{sessions:{email:"joe@dbz.edu", password:"password"}}
			expect(assigns(:user)).to be_a User
		end
		context "the user info is valid" do
			it 'stores the users id in the session' do
				post :create, {sessions:{email:"joe@dbz.edu", password:"password"}}
		...
		context "The user info isn't valid" do
			it "renders the login form" do
				post :create, {sessions:{email:"j@d.edu", password:"pass"}}
				expect(response).to render_template "new"
			end
		end
	end
